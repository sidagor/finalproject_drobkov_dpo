...
from .utils import (
    load_json, save_json, get_user_by_username, load_portfolio,
    save_portfolios,parse_args, load_rates,get_rate_rates, SESSION, USERS_FILE, PORTFOLIOS_FILE
)
...
def buy(args):
    ...
    rate = get_rate_rates(currency, "USD")
    if rate is None:
...
def get_rate(args):
    ...
    rate = get_rate_rates(c_from, c_to)
    if rate is None:
